  ...
  den.aspects.firefox.homeManager = {
    ...
    options.firefox = {
      profileName = lib.mkOption {
        ...
        description = "Default Firefox profile name.";
      };
    ...
    config = {
      programs.firefox = {
        ...
        policies = lib.recursiveUpdate config.firefox.policies {
          ...
          Bookmarks =
            config.firefox.bookmarks
            ++ [
              {
                Folder = "Dreamland";
                Title = "Luanti (Navy Industry)";
                URL = "https://luanti.navi";
              }
            ];
        };

        profiles."${config.firefox.profileName}" = {
          isDefault = true;
          id = 0;
          ...
          search = {
            ...
            engines = {
              ...
              "DaFont" = {
                urls = [{template = "https://www.dafont.com/search.php?q={searchTerms}";}];
                definedAliases = ["@dafont" "@font"];
              };
              "ChatGPT" = {
                urls = [{template = "https://chatgpt.com/?temporary-chat=true&q={searchTerms}";}];
                definedAliases = ["@chatgpt" "@gpt"];
              };
              "youtube" = {
          ...
          settings = lib.recursiveUpdate config.firefox.settings {
            ...
            # Disable hidden cursing during loading page
            "dom.event.pointer.boundary.dispatch_when_layout_change" = false;
      ...
      home.file.".mozilla/firefox/profiles.ini".text = ''
        [Profile0]
        Name=${config.firefox.profileName}
        IsRelative=0
        Path=${config.home.homeDirectory}/.config/mozilla/firefox/${config.firefox.profileName}
        Default=1
