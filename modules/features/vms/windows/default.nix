{
  den.aspects.vm-nightmare.nixos = {
    pkgs,
    nixvirt,
    lib,
    config,
    ...
    config = {
      system.activationScripts.copyNvramWindows.text = ''
        dest="/var/lib/libvirt/qemu/nvram/windows_VARS.fd"
        [ -e "$dest" ] || {
          mkdir -p "$(dirname "$dest")"
          cp ${(pkgs.OVMF.override {
          secureBoot = true;
          msVarsTemplate = true;
        }).fd}/FV/OVMF_VARS.ms.fd "$dest"
          chown qemu-libvirtd:kvm "$dest"
          chmod 660 "$dest"
        }
      '';
                  ...
                  // {
                    ...
                    devices =
                      ...
                      // {
                        ...
                        disk =
                          lib.optionals (config.windows.iso != null) [
                            {
                              type = "file";
                              device = "cdrom";
                              driver = {
