        ...
        overlays = [
          (_: prev: {
            adi1090x-plymouth-themes = prev.adi1090x-plymouth-themes.overrideAttrs (old: {
              # cp ${pkgs.nixos-artwork.wallpapers.nineish}/share/backgrounds/nixos/nix-wallpaper-nineish.png $out/share/plymouth/themes/${config.boot.plymouth.theme}/nixos-logo.png
              installPhase =
                (old.installPhase or "")
                + ''
